/**
 * 文件：backend/src/schemas/persona.ts
 * 功能描述：DigitalPersona v2 角色模型 Zod Schema（静态设定 + 运行时状态）
 * 作者：Haotian Chen · 版本：v1.0.0
 */
import { z } from 'zod';

/**
 * 允许 persona meta.id 使用 UUID 或友好 slug，避免非 UUID 数据无法通过验证
 */
const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const slugRegex = /^[-A-Za-z0-9_]{3,128}$/;
const personaIdSchema = z
  .string()
  .min(3)
  .max(128)
  .refine((value) => uuidRegex.test(value) || slugRegex.test(value), {
    message: 'meta.id must be a UUID or slug (alphanumeric, hyphen, underscore)'
  });
