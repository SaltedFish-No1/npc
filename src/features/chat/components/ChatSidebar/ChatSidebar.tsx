...
import { TEXT_MODEL_NAME } from '@/config/constants';
...
export function ChatSidebar({
  ...
  <img src={avatar} alt={CHARACTER_PROFILE.codename} className={styles.avatarImage} />
      ...
      <div className={styles.panelFooter}>
        <span>
          {t('chat.sidebar.engineLabel')}: {apiKey ? 'DOUBAO AI' : t('chat.sidebar.pendingLabel')}
        </span>
        <span style={{ fontSize: 10 }}>{t('chat.sidebar.textModelLabel', { model: TEXT_MODEL_NAME })}</span>
      </div>
    </aside>
  );
}
